...
def Operating_list():
  while True:
    ...
    if a == "1":
      print("你選擇的是查詢所有員工資料")
      sql_get_all_staff_info() 
    elif a == "11":
      print("你選擇的是查詢員工資料")
      sql_get_one_staff_info()
    elif a == "2":
      print("你選擇的是修改員工資料")
      staff_data_update()
    elif a == "3":
...
def staff_data_register():
  while True:
    ...
    data,acc = sql_check_acc_existed()
...
def staff_data_delete():  # 刪除有兩種 ->   修正狀態碼 0->1  / 直接完整刪掉(不留歷史資料)
  while True:
    ...
    if option == data[1]: # 用option比對 先前的data[1](帳號名稱)
      # 讓SQL自行尋找要刪除的帳號
      sql_delete="DELETE FROM staff_info WHERE sf_account = '" +option+"' "
...
def sql_get_all_staff_info(): # 查詢全部員工資料 可運行
  sql = "SELECT sf_pk,sf_name,sf_account,sf_pwd,sf_level,sf_del FROM staff_info WHERE sf_del = 0 "
  cur.execute(sql)
  staff_data = cur.fetchall()
  print("員工資料表:(編號、姓名、帳號、密碼、狀態碼)")
  print(staff_data)
  return staff_data

def sql_get_one_staff_info(): # 查詢特定員工資料 可運行
  acc = input("請輸入帳號:")
  if acc == "" : 
    print("你輸入為空值，請重新執行。")

  sql = " SELECT sf_name,sf_account,sf_pwd FROM staff_info WHERE sf_account='"+ acc +"' AND sf_del = '0' " 
  cur.execute(sql) # 資料庫執行抓資料存在cur裡  # sql 裡面抓變數用"+變數+"
  data = cur.fetchone() # print(staff_acc) # 總共抓了 名字、帳號、密碼三個欄位
  print("員工資料表:(姓名、帳號、密碼)")
  print(data)
  return data
...
#   def delete():
#     acc = input("請輸入帳號:")
#     sql_delete="DELETE FROM staff_info WHERE sf_acc = '" + acc +"' "
#     cur.execute(sql_delete)
#     conn.commit() # commit 確認提交(用於資料庫"更新")
