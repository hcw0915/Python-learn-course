...
身高%.1fcm，標準體重介於%.2f~%.2fkg" % (height, rcmd_low, rcmd_up))
	...
	def promote(self):
		...
		def promote_choice(score_default, i):  # !!!!已排除輸入不同字串、符號、數字的情況!!!!
			while True:
				...
				if str.isdigit(answer) == True:  # 判定輸入的answer是否為數字
					print("回復錯誤，請重新作答。\n")
					continue
	...
	def hangman(self):
		...
		while anwser != question_copy:    # 兩個不相等代表還沒有猜中
			...
			list_guess.append(alphbet)
			# 用set轉換list 因為set裡有不重複性，所以會把重複的刪掉
			set_guess = set(list_guess)
			list_guess = list(set_guess)                    # 再把set轉回list列表
			if alphbet in list_question:
				# 數 list_qusetion 裡面有多少個alphbet
				count_right = list_question.count(alphbet)
